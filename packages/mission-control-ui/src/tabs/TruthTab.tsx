    ...
    <>
      <div className="mc-card">
        ...
        <p style={{ color: "var(--fc-fg-muted)", fontSize: 12, margin: "0 0 12px" }}>
          Generates a single canonical <code>TRUTH.md</code> at the venture root stating invariants,
          assumptions, and risks. Routed through the Phase 2 handoff dispatcher
          (GENERATE_TRUTH_LAYER bundle).
        </p>
        <div className="mc-grid" style={{ gridTemplateColumns: "auto 1fr" }}>
          ...
          />
          <label htmlFor="truth-scope" style={{ alignSelf: "start", marginTop: 6 }}>
            Scope notes
          </label>
          <textarea
            id="truth-scope"
      ...
      {result && (
        <div className="mc-card">
          ...
          {result.summary && <p style={{ marginTop: 8 }}>{result.summary}</p>}
          {result.producedArtifacts && result.producedArtifacts.length > 0 && (
            <p style={{ marginTop: 8, color: "var(--fc-fg-muted)", fontSize: 12 }}>
              ...
              {result.producedArtifacts.map((a) => (
                <code key={a} style={{ marginRight: 6 }}>
                  {a}
                </code>
              ))}
            </p>
          ...
          {result.body ? (
            ...
            <p style={{ marginTop: 8, color: "var(--fc-fg-muted)", fontSize: 12 }}>
              No <code>TRUTH.md</code> on disk after the run. Check the Output panel →
              "founder-cowork" channel for runner logs.
            </p>
          )}
        </div>
      )}
