    ...
    <>
      <div className="mc-card">
        <div className="mc-row" style={{ justifyContent: "space-between" }}>
          <h3 className="mc-card-title" style={{ margin: 0 }}>Registered agents</h3>
          <button
            className="secondary"
        ...
        <table className="mc-table" style={{ marginTop: 8 }}>
          ...
          <tbody>
            {props.agents.map((a) => {
              ...
              const status =
                h === undefined ? <span className="mc-pill">pending</span> :
                h.healthy ? <span className="mc-pill success">ready</span> :
        ...
        <p style={{ marginTop: 8, color: "var(--fc-fg-muted)", fontSize: 12 }}>
          A red status means the binary isn't on PATH. Install the CLI and log in,
          then re-run preflight. Spawning an unhealthy agent is allowed but
      ...
      <div className="mc-card">
        ...
        <p style={{ color: "var(--fc-fg-muted)", margin: 0 }}>
          Per-agent account switching (Codex managed-account pattern, Claude
          system-default snapshot, Gemini SecretStorage) lands in Phase 3 with
          packages/agent-accounts. For now, agents use whichever credentials
          their CLI is logged in with.
        </p>
