...
export function useHandoffWatcher(ventureRoot: string | null | undefined): void {
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      const onResult = await listen<HandoffEventPayload>(
        "handoff:result",
        (evt) => {
          ...
          try {
            ...
            if (!r.success) {
              log.warn(
                "Bad HandoffResult from " + evt.payload.runId +
                  ": " + r.error.message,
              );
            }
          } catch (e) {
            log.warn(
              "Bad JSON in handoff:result for " + evt.payload.runId +
                ": " + String(e),
            );
            return;
          }
      ...
      const onProgress = await listen<HandoffEventPayload>(
        "handoff:progress",
        (evt) => {
          ...
          try {
            ...
            if (
              ...
              log.warn(
                "Bad HandoffProgressEvent shape for " + evt.payload.runId,
              );
            }
          } catch (e) {
            log.warn(
              "Bad JSON in handoff:progress for " + evt.payload.runId +
                ": " + String(e),
            );
          }
        },
      ...
      const onError = await listen<string>(
