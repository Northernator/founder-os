/**
 * pickActiveCrmProvider -- walks the configured tier list and returns
 * the first provider whose available() resolves true.
 *
 * config_only is always available, so the resolver never returns null
 * as long as the tier list ends with config_only. Tier lists that
 * accidentally omit it can still return null -- callers should defend
 * against that explicitly.
 *
 * Mirrors the @founder-os/media-providers resolver shape, but trimmed
 * for the simpler CRM provider surface (no engine-hint per-shot decisions,
 * no auto-tier signal mapping).
 */
...
export type ResolverInput = {
  /**
   * Ordered list of providers to try, by engine name.
   */
  tierList: ReadonlyArray<CrmEngine>;
  /**
   * Map from engine name to a CrmProvider instance. Providers not in
   * the map are silently skipped (lets callers pass an incomplete set
   * during slice 5a when the WebView only has config_only).
   */
...
export type ResolverResult = {
   ...
   * reported available=true (which is only possible if config_only
   * isn't in the tier list or its available() throws).
   */
...
export async function pickActiveCrmProvider(
  input: ResolverInput
